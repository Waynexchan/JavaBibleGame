...
public class Main extends JPanel implements KeyListener {
    ...
    public void paintComponent(Graphics g){
        gameView.drawView(g);
        moses.draw(g); //引用左Sprite內draw method
    ...
    public static void main(String[] args) {
        ...
        window.setResizable(false);//遊戲進行中不能改變視窗大小
    }
    private boolean changeLevel(String result){ //設定boolean值 是否changeLevel
        if (result.equals("Next level")){//如果result = Next level ,變true 過關
            level++;
            if (level == 2){
    ...
    public void keyPressed(KeyEvent e) {
        ...
        switch (e.getKeyCode()){
            case KeyEvent.VK_UP:
                if (mosesPoint.y > 1){
                    String result = moses.overlap(mosesPoint.x, mosesPoint.y-1);//拎取移動後既位置放入Moses overlap method
                    ...
                    if (!result.equals("Cannot Move")){//如果唔係唔可以郁動既情況
                        mosesPoint.y -= 1;
                    }
            ...
            case KeyEvent.VK_LEFT:
                if (mosesPoint.x > 1){
                    String result = moses.overlap(mosesPoint.x - 1, mosesPoint.y);//拎取移動後既位置放入Moses overlap method
                    if (result.equals("Die")){
                        //reset game
                        level = 1;
                        JOptionPane.showMessageDialog(this,"You die, please try again!");
            ...
            case KeyEvent.VK_RIGHT:
                if (mosesPoint.x < COLUMN){
                    ...
                    if (!result.equals("Cannot Move")){//如果唔係唔可以郁動既情況, x會+=1 ,但遇到就唔會執行x+=1
                        mosesPoint.x += 1;
                    }
                    if (result.equals("Game Over")){
                        JOptionPane.showMessageDialog(this,"You win the Game!");
